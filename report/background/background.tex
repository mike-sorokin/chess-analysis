...
identify this type of tactic, as a checkmate delivered to a king on the back rank is not necessarily a \emph{back-rank checkmate}.
...
\begin{figure}[H]
    \begin{minipage}{0.475\textwidth}
        ...
        \caption{A trivial back-rank checkmate, white mates with \textbf{Rb8\#}
...
TODO: Explain chess FEN, PGN notation. The reader is assumed to know how to
...
In ``A description language for chess'',\cite{chessLanguage} López-Michelone
et al\@. describe a language to search across chess positions. The main features
...
language as ``kg8, pf7, pg7, B(ph7), Nf3, Qd1, Pe5''\footnote{This string
corresponds to a black king on g8, black pawns on f7, g7, h7, a white bishop
attacking h7, a white knight on f3 (ready to deliver a check with
\textbf{Ng5+}, a common motif in \emph{Greek gift sacrifices}), a white queen
...
After \textbf{16...Be7 17. O-O}, Black blundered with \textbf{17...Bxa3??},
after which, the \emph{Greek gift sacrifice} (\textbf{18. Bxh7!!}, shown in
...
these positions, White has the brilliant move \textbf{1. Qh6+!!}, following
with \textbf{2. Rh8\#} if \textbf{1...Kxh6}, and either \textbf{2. Rf7\#} or
\textbf{2. Rb7+} (leading to a quick mate) if \textbf{1...gxh6}. 

This pattern, whilst very rare, is undeniably identical between the 2 games.
The unavailability of the \textbf{g6} square to the enemy king, combined with
...
Miroslav et al\@. report their findings on a program \cite{middlegamePatterns} to
identify the \emph{Greek gift sacrifice} in chess middlegames\footnote{A
hard-to-quantify phase of the chess game where most pieces are developed and
the kings are positioned away from danger.}. They found success, partly thanks
to their detailed representatioon of the board, where each square is
represented by 71 binary attribites \cite{middlegamePatterns} to encode the
...
Miroslav et al\@. achieved an 87.7\% classification performance on detecting
...
In this study, Turker et al\@. propose novel kernels for efficient feature
