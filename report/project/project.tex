...
Similar to the work of \citet{chessTrees}, Stockfish was used to generate
meaningful trees of a maximum depth of 5 ply. Each level of the tree has a
maximum of 3 possible moves that are within 100 centipawns (arbitrarily chosen)
of the best move. If all 3 moves for the side solving the puzzle are within
this range, then the puzzle is considered solved; \citet{chessTrees} writes:
``This means that the position is already so strong that it no longer matters
what move the player chooses because almost everything wins.''. Finally, a
checkmate naturally indicates that the puzzle is solved. 
