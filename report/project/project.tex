...
encoding (\Cref{mlS1}) and ending with the training, validation, and testing
splits (\Cref{mlS12}).
...
With this, we seek to develop a model that, given a stationary chess position
in FEN (see Subsection \ref{fenSection}), returns a prediction for this position's
puzzle label vector.
...
solve this, we use $12\times8\times8$ chess bitboards, inspired by the work of
\citet{chess2vec}. Slightly more formally, the element $B[z, x, y]$ of bitboard
$B$ is $1$ iff there is a piece $z \in \{\texttt{p}, \texttt{P}, \texttt{n},
\texttt{N}, \texttt{b}, \texttt{B}, \texttt{r}, \texttt{R}, \texttt{q},
\texttt{Q}, \texttt{k}, \texttt{K}\}$\footnote{FEN-inspired -- white pieces are
uppercase, black pieces are lowercase.} at rank $x \in \{\texttt{1},
\texttt{2}, \texttt{3}, \texttt{4}, \texttt{5}, \texttt{6}, \texttt{7},
\texttt{8}\}$}, file $y \in \{\texttt{a}, \texttt{b}, \texttt{c}, \texttt{d},
...
Since the clustering method described in Subsections \ref{treeS22} produces too
many outliers to be useful for inference, we can use a classic $k$-nearest
neighbours approach \citep{fix1985discriminatory} for both classification of
puzzle themes and regression of puzzle Elo difficulty.

We have shown that the novel distance function, as described in Subsection
\ref{treeS13}, has promising results for finding similar positions to a puzzle
(Subsection \ref{treeS21}).

\subsection{Search for the Best $k$)
